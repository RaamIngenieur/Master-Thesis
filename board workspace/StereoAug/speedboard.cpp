...
void census11(unsigned int * P, ucarray &b, int r, int c)
{
	int x;
	unsigned int * Q;
...
void SHDR2L13(unsigned int* r, uint64x2_t  *left, uint64x2_t  *right, int row, int column, unsigned int* hdis, unsigned int* x)
{
	...
	for (int drange = dmax; drange >= dmin; drange--)
	{
		...
		for (int i = 0; i < row; i++)
		{
			for (int j = 0; j < column - drange; j++)
			{
				...
				*w+= __builtin_popcountl(*(sb+2));
				*w+= __builtin_popcountl(*(sb+3));
				lp++;
				rp++;
				w++;
			}
...
int main()
{
	...
	LC = (unsigned int*)malloc(row * column* 4* sizeof(unsigned int ));
	RC = (unsigned int*)malloc(row * column* 4* sizeof(unsigned int));
	if (LC == NULL||RC == NULL)
	{
		fprintf(stderr, "out of memory\n");
	}
	...
	std::thread a(SHDR2L13, O, (uint64x2_t*)LC,(uint64x2_t*) RC, row / 4 + 6, column,hdis,x);
	std::thread b(SHDR2L13, O + (row / 4 - 6)*column, (uint64x2_t*)LC + (row / 4 - 6)*column, (uint64x2_t*)RC + (row / 4 - 6)*column, row / 4 + 12, column,hdis + (row / 4 - 6)*column,x + (row / 4 - 6)*column);
	std::thread c(SHDR2L13, O + (row / 2 - 6)*column, (uint64x2_t*)LC + (row / 2 - 6)*column, (uint64x2_t*)RC + (row / 2 - 6)*column, row / 4 + 12, column,hdis + (row / 2 - 6)*column,x + (row / 2 - 6)*column);
	std::thread d(SHDR2L13, O + (3 * row / 4 - 6)*column, (uint64x2_t*)LC + (3 * row / 4 - 6)*column, (uint64x2_t*)RC + (3 * row / 4 - 6)*column, row / 4 + 6, column, hdis + (3 * row / 4 - 6)*column, x + (3 * row / 4 - 6)*column);
