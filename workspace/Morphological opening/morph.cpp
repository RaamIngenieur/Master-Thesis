...
void census13(unsigned long * P, ucarray &b, int r, int c)
{
	int x;
	unsigned long * Q;
...
void SHDR2L13(float* r, unsigned long  *left, unsigned long  *right, int row, int column)
{
	unsigned int hdis = 28561, dif1, dif2, dif3;
	int dmin=0, dmax=100;
	float num, den, pos;
	int y;
	unsigned long  *lp,*rp,x;
	...
	for (int i = 0; i < row - 12; i++)
	{
		for (int j = 0; j < column - 12; j++)
		{
			for (int drange = dmax; drange >= dmin; drange--)
			{
				...
				{
					y = 0;
					lp = left + (i*column + j + drange) * 3;
					rp = right + (i*column + j) * 3;
					for (int k = 12; k >= 0; k--)
					{
						for (int l = 38; l >= 0; l--)
						{
...
int main()
{
	...
	LC = (unsigned long*)malloc(row * column* 3* sizeof(unsigned long ));
	RC = (unsigned long*)malloc(row * column*3*sizeof(unsigned long));
	if (LC == NULL||RC == NULL)
	{
		fprintf(stderr, "out of memory\n");
	}
	...
	std::thread b(SHDR2L13, O + (row / 4 - 6)*column, LC + (row / 4 - 6)*column*3, RC + (row / 4 - 6)*column*3, row / 4 + 12, column);
	std::thread c(SHDR2L13, O + (row / 2 - 6)*column, LC + (row / 2 - 6)*column * 3, RC + (row / 2 - 6)*column * 3, row / 4 + 12, column);
	std::thread d(SHDR2L13, O + (3 * row / 4 - 6)*column, LC + (3 * row / 4 - 6)*column * 3, RC + (3 * row / 4 - 6)*column * 3, row / 4 + 6, column);
	...
	__m128i m=_mm_set1_epi32 (1),n=_mm_set1_epi32 (5),o;

	o=_mm_xor_si128 (m, n);
	int x = _mm_extract_epi16 (o, 0);
	cout<<endl<<x<<endl;
	...
	LO.arraytoimage(&imageL);
	RO.arraytoimage(&imageR);

	imageL.write("outputL.png");
	imageR.write("outputR.png");
