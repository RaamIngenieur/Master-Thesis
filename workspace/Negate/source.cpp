...
void negate1(unsigned int *P,int row, int column)
{
  for (int i = 6; i < row-6; i++)
  {
    for (int j = 6; j < column-6; j++)
    {
       *(P + i*column +j)=255-*(P + i*column +j);
    }
  }
}
...
int main()
{
  ...
  unsigned int *left, *right, *result;

  left = (unsigned int*)malloc(row * column * 6 * sizeof(unsigned int ));
  right = (unsigned int*)malloc(row * column * 6 * sizeof(unsigned int));
  result = (unsigned int*)malloc(row * column * sizeof(unsigned int));
  if (left == NULL||right == NULL || result == NULL)
  {
    fprintf(stderr, "out of memory\n");
  }

 //preprocessimage(&imageL, L, LO, left, row,column);
 //preprocessimage(&imageR, R, RO, right, row, column);

  /*for (int i = 0; i < row; i++)
  {
    for (int j = 0; j < column; j++)
    {
      *(left+i*column+j) = (unsigned int)imageL[i][j];
    }
  }
  */
  ...
  //nema.shd(result,left,right,row,column);
  negate1(result,row,column);
