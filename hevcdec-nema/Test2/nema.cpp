...
Nema::init(int row, int column)
{
  ...
  tsi_cmdl_add_cmd(NEMA_TARGET_CLIPMAX, ((m_yres) << 16) | (m_xres));
...
void Nema::negate(unsigned int  *pic, int row, int column,bool inited)
{
  if (inited)
  {
    int width, height;
    ...
    tsi_cmdl_add_cmd(NEMA_VERTEX1_X, width);
    tsi_cmdl_add_cmd(NEMA_VERTEX1_Y, height);
