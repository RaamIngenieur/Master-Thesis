...
int main()
{
  ...
  unsigned int *lct, *rct, *hdis, *exoradd, *result;
 ...
 // nema.wait_for_interrupt();
  ...
  {
    nema.exoradd(exoradd, lct, rct, row, column, drange, drange==dmax);
    //  nema.wait_for_interrupt();

    nema.shd(exoradd,hdis,result,row,column,drange, false);
    //  nema.wait_for_interrupt();
  }
