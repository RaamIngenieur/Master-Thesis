...
void Nema::exoradd(unsigned int* exoradd, unsigned int  *lct, unsigned int  *rct, int row, int column, int drange)
{
  int width, height;
  /// Now that the video dimensions are known reprogram rasterizer and the interpolators
  width  = column;
...
void Nema::shdy(unsigned int* exoradd, unsigned int* HammDistY, int row, int column,int drange)
{
  int width, height;
  ...
  /// Load binary
  void *bin = nema_load_binary("kernels/shdy.bin");
  assert(bin);
  ...
  tsi_cmdl_add_cmd(NEMA_VAR0_V0_1, (uint32_t)nema_virt_to_phys((uint32_t)HammDistY)); /// Interpolator 1 start
...
void Nema::shd(unsigned int* HammDistY, unsigned int* hdis, unsigned int *result, int row, int column,int drange)
{
  int width, height;
  ...
  /// Setup Interpolators
  ///v0.w
  tsi_cmdl_add_cmd(NEMA_VAR0_V0_0, (uint32_t)nema_virt_to_phys((uint32_t)HammDistY)); /// Interpolator 0 start
